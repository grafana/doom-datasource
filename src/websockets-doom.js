export function init(Module) {
  ...
  wasmBinaryFile = '/public/plugins/grafana-doom-datasource/img/websockets-doom.wasm';
    ...
    try {
      ...
      while (idx < stream.getdents.length && pos + struct_size <= count) {
        ...
        (tempI64 = [
          ...
          SAFE_HEAP_STORE((dirp + pos) | 0, tempI64[0] | 0, 4),
          SAFE_HEAP_STORE((dirp + pos + 4) | 0, tempI64[1] | 0, 4);
        (tempI64 = [
          ((idx + 1) * struct_size) >>> 0,
          ...
          SAFE_HEAP_STORE((dirp + pos + 8) | 0, tempI64[0] | 0, 4),
          SAFE_HEAP_STORE((dirp + pos + 12) | 0, tempI64[1] | 0, 4);
  ...
  function __mmap_js(addr, len, prot, flags, fd, off, allocated, builtin) {
    try {
      ...
      var ptr = res.ptr;
      SAFE_HEAP_STORE(allocated | 0, res.allocated | 0, 4);
      return ptr;
    } catch (e) {
      if (typeof FS == 'undefined' || !(e instanceof FS.ErrnoError)) throw e;
      return -e.errno;
  ...
  function readAsmConstArgs(sigPtr, buf) {
    ...
    while ((ch = SAFE_HEAP_LOAD(sigPtr++, 1, 1))) {
      ...
      readAsmConstArgsArray.push(
        readAsmConstArgsDouble ? SAFE_HEAP_LOAD_D((buf++ >> 1) * 8, 8, 0) : SAFE_HEAP_LOAD(buf * 4, 4, 0)
      );
      ++buf;
    }
    ...
    if (deserialized != num)
      warnOnce(
        'writeI53ToI64() out of range: serialized JS Number ' +
          ...
          SAFE_HEAP_LOAD((ptr >> 2) * 4, 4, 1).toString(16) +
          ', hi=0x' +
          SAFE_HEAP_LOAD(((ptr + 4) >> 2) * 4, 4, 1).toString(16) +
  ...
  function _emscripten_glVertexAttrib3fv(index, v) {
    GLctx.vertexAttrib3f(
      index,
      SAFE_HEAP_LOAD_D((v >> 2) * 4, 4, 0),
      SAFE_HEAP_LOAD_D(((v + 4) >> 2) * 4, 4, 0),
      SAFE_HEAP_LOAD_D(((v + 8) >> 2) * 4, 4, 0)
    );
  }
  ...
  if (!Object.getOwnPropertyDescriptor(Module, 'Asyncify'))
    Module['Asyncify'] = () => abort("'Asyncify' was not exported. add it to EXPORTED_RUNTIME_METHODS (see the FAQ)");

  if (!Object.getOwnPropertyDescriptor(Module, 'Fibers'))
    Module['Fibers'] = () => abort("'Fibers' was not exported. add it to EXPORTED_RUNTIME_METHODS (see the FAQ)");

  if (!Object.getOwnPropertyDescriptor(Module, 'WS'))
    Module['WS'] = () => abort("'WS' was not exported. add it to EXPORTED_RUNTIME_METHODS (see the FAQ)");
