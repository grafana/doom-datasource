...
static int max_scaling_buffer_pixels = 3200000;
...
static double colorDistance(int r, int g, int b, int c2) {
  const int deltaR = r - pall[c2][0];
  const int deltaG = g - pall[c2][1];
  const int deltaB = b - pall[c2][2];
  const double redmean = (r + pall[c2][0]) / 2;
  return sqrt(
    (2 + redmean / 256) * pow(deltaR, 2) +
      4 * pow(deltaG, 2) +
      (2 + (255 - redmean) / 256) * pow(deltaB, 2)
  );
}
...
static void LimitTextureSize(int *w_upscale, int *h_upscale)
{
    ...
    while (*w_upscale * 320 > rinfo.max_texture_width)
    {
        --*w_upscale;
    }
    while (*h_upscale * 200 > rinfo.max_texture_height)
    {
        --*h_upscale;
    }
    ...
    if (max_scaling_buffer_pixels < 320 * 200)
    {
        I_Error("CreateUpscaledTexture: max_scaling_buffer_pixels too small "
                "to create a texture buffer: %d < %d",
                max_scaling_buffer_pixels, 320 * 200);
    }

    while (*w_upscale * *h_upscale * 320 * 200
...
static void CreateUpscaledTexture(boolean force)
{
    ...
    w_upscale = (w + 320 - 1) / 320;
    h_upscale = (h + 200 - 1) / 200;
...
void I_FinishUpdate (void)
{
	...
	for (i=0 ; i<tics*4 ; i+=4)
	    I_VideoBuffer[ (200-1)*320 + i] = 0xff;
	for ( ; i<20*4 ; i+=4)
	    I_VideoBuffer[ (200-1)*320 + i] = 0x0;
    }
    ...
    for (int i = 0; i < 320; i++) {
        for (int j = 0; j < 200; j++) {
            SDL_GetRGB(getpixel(screenbuffer, j, i), screenbuffer->format, &color.r, &color.g, &color.b);
            a = getColorIndex(color.r, color.g, color.b);
...
{
    memcpy(scr, I_VideoBuffer, 320*200*sizeof(*scr));
}
...
static void SetVideoMode(void)
{
    ...
    if (screen == NULL)
    {
        ...
        SDL_SetWindowMinimumSize(screen, 320, 200);
...
void I_InitGraphics(void)
{
    ...
    memset(I_VideoBuffer, 0, 320 * 200 * sizeof(*I_VideoBuffer));
